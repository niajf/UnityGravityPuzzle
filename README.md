...
マウスクリックで重力方向を6軸から切り替え、プレイヤーをゴールへと導くことがコンセプトです。
ゲームプレイの面白さだけでなく、**拡張性・保守性を意識したアーキテクチャ設計**を示すことを目的としています。

![Gameplay Demo](videos/demo.gif)
...
### 8. Input System による入力管理 (New Input System)

旧 Input Manager から Unity 6 標準の Input System へ完全移行しました。
Input Action Asset でアクションを定義し、キーバインドをデータとして管理する設計に変更。
ゲームパッドやタッチ入力への拡張が容易なアーキテクチャになっています。

```
InputManager (Singleton)
  └── PlayerInputActions (Action Asset)
        ├── Player/Move      → PlayerController, CameraFollow
        ├── Player/Look      → PlayerController, CameraFollow
        ├── Player/ToggleZoom → CameraFollow
        └── Player/LeftGravity / RightGravity → GravityManager
```

### 9. ScriptableObject によるデータ駆動設計 (Data-Driven Design)

ドローンAI・プレイヤー・ステージの各パラメータを ScriptableObject に分離し、コード変更なしに Inspector 上でバランス調整が可能な設計を実現しました。

| ScriptableObject | 管理するパラメータ |
|---|---|
| `DroneConfig` | 移動速度・視野角・索敵距離・攻撃間隔など |
| `PlayerConfig` | 移動速度・回転速度・カメラ距離・感度など |

### 10. オブジェクトプーリング (Object Pooling)

ドローンが射出する弾に Unity 標準の `ObjectPool<T>` を適用し、Instantiate / Destroy による GC アロケーションを排除しました。

- `ProjectilePool` が生成・取得・返却を一元管理し、`Projectile` 側はプールの実装を意識しない設計

### 11. UniTask による非同期処理 (Async/Await with UniTask)

シーン遷移・フェード演出を UniTask の async/await で実装し、コルーチンと比較してゼロアロケーションかつ堅牢なエラーハンドリングを実現しました。

```csharp
// TitleManager.cs — destroyCancellationToken でオブジェクト破棄時に自動キャンセル
await CanvasGroupFader.FadeAsync(fader, 0f, 1f, 1f, destroyCancellationToken);
await SceneManager.LoadSceneAsync(nextSceneIndex);
```

- `destroyCancellationToken` でオブジェクトのライフサイクルに連動したキャンセルを実現
- フェード中のシーン破棄でも `MissingReferenceException` が発生しない安全な実装
...
│   ├── InputManager.cs        # Input System の一元管理
│   ├── SoundManager.cs        # サウンド管理
│   └── TagScript.cs           # 子オブジェクトへの Tag 一括適用
├── Player/
│   ├── PlayerController.cs    # 移動・姿勢制御
│   ├── CameraFollow.cs        # サードパーソンカメラ
│   └── PlayerInputActions.cs  # Input Action Asset 自動生成クラス
├── Enemy/
│   ├── DroneAI.cs             # ステートマシン敵AI
│   ├── Projectile.cs          # 弾の挙動・プール返却
│   └── ProjectilePool.cs      # オブジェクトプール管理
├── Stage/
│   ├── GoalArea.cs            # ゴール判定
│   ├── KillZone.cs            # 即死ゾーン
│   ├── OutStage.cs            # ステージ外脱出検知
│   ├── PressureSwitch.cs      # UnityEvent トリガー
│   └── Door.cs                # Lerp 開閉アニメーション
├── Config/
...
- **弾のオブジェクトプール:** GC アロケーションゼロの射撃システム
- **ステージギミック:** 圧力スイッチ連動ドア（Lerp アニメーション）
- **ゴール・死亡判定:** コライダートリガーによるゲーム状態遷移
- **HUD:** TextMeshPro による重力方向表示・クリアタイム計測
- **タイトル画面:** UniTask フェードイン、浮遊アニメーション演出
- **Input System:** Action Asset によるキーバインド管理・ゲームパッド対応可能な設計
- **ScriptableObject:** Inspector からコード変更なしにパラメータ調整可能
